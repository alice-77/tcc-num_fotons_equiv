...
\begin{figure}[h]
	...
	\captionsetup{width=\textwidth}
	\input{./figs/passing_charge_tikz.tex}
...
O método dos fótons equivalentes consiste em aproximar a interação desses
...
após a colisão \cite{povh6ed}. Também definimos a função de distribuição de
carga $f(\vec{r})$, tal que a densidade de carga seja,
\begin{equation}
...
\begin{table}[h]
	\IBGEtab{
		\caption{Diferentes fatores de forma obtidos para diferentes
		distribuições de carga esfericamente simétricas.}
		\label{tab_charge-distribution}
	}{
		\input{./tables/form_factors.tex}
	}{
...
\begin{table}[h]
	\IBGEtab{
		...
		\input{./tables/collision_labs.tex}
	}{
